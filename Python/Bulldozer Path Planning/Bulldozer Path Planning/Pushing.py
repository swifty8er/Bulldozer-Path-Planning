...
class Pushing:
    ...
    @staticmethod
    def continuousPushDistance(push_point,curr_disk_pos,distance,curr_disk_positions,disk_being_pushed,map):
        ...
        if not Pushing.pushingCollision(push_point,new_disk_pos,curr_disk_positions,disk_being_pushed,map):
    ...
    @staticmethod
    def pushDisk(push_point,curr_disk_pos,closestGoal,curr_disk_positions,disk_being_pushed,map,max_distance=(0.4*math.pi)/4.0,num_steps=50):
        bestPush = None
        min_dist = math.inf
        found = False
        if BasicGeometry.ptDist(closestGoal,curr_disk_pos) < max_distance:
            ...
            for i in range(num_steps):
                ...
                if dist < min_dist and not Pushing.pushingCollision(push_point,new_disk_pos,curr_disk_positions,disk_being_pushed,map):
                    min_dist = dist
    ...
    @staticmethod
    def pushingCollision(push_point,disk_pos,curr_disk_positions,disk_being_pushed,map):
        ...
        for edge in edges:
            ...
            if map.disk_radius - BasicGeometry.point2LineDist(edge,disk_pos) > np.finfo(np.float32).eps:
                return True
        for i in range(len(curr_disk_positions)):
            if i == disk_being_pushed:
                continue
            curr_pos = curr_disk_positions[i]
    ...
    @staticmethod
    def getPushPointsFromHeatmap(disk_pos,closest_goal,curr_disk_positions,disk_being_pushed,map,curr_heading=-1):
        push_points = []
        ...
        while alpha <= max_angle:
            ...
            collisionWithDisk = False
            for i in range(len(curr_disk_positions)):
                if i == disk_being_pushed:
                    continue
                curr_pos = curr_disk_positions[i]
                if 2*map.disk_radius - BasicGeometry.ptDist(curr_pos,push_point) > np.finfo(np.float32).eps:
                    collisionWithDisk = True
                    break
            if not collisionWithDisk:
