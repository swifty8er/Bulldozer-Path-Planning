            ...
            if pose not in visitedNodes:
                ...
                for next_pose in self._RRT.tree[pose]:
                    if not next_pose in visitedNodes:
                       if len(path)>0 and not self._RRT.edgeCollidesWithDirtPile(pose,next_pose,self._RRT.tree[pose][next_pose],self._curr_disk_positions):
    ...
    def getContinuousAnglePushState(self,curr_disk_pos,closest_goal,disk_being_pushed,ax=False):
        ...
        if self._RRT.connectPushPoint(push_point,curr_disk_pos,self._curr_disk_positions):
            ...
            if not (curr_disk_pos[0] == new_disk_pos[0] and curr_disk_pos[1] == new_disk_pos[1]):
                ...
                new_pushed_disks.append(disk_being_pushed)
                #use A* where g = full path length
                (connected,fullPathLength) = self.connectTwoPoses(new_vehicle_pose,self._vehicle_pose,ax)
                if not connected:
                    if not self.growBidirectionalRRTToConnectPoses(new_vehicle_pose,self._vehicle_pose,ax):
                        return False
                    (connected,fullPathLength) = self.connectTwoPoses(new_vehicle_pose,self._vehicle_pose,ax)
                    if not connected:
                        return False
